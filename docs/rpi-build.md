...
* После обновления пакетов или ядра на Raspberry Pi **обязательно** запускайте
  `./rsync.sh`, чтобы актуализировать sysroot и пересобрать Docker-образ.
* По возможности храните `Dockerfile`, `rsync.sh` и `cmake/toolchains/rpi-arm64.cmake`
  прямо в репозитории проекта — это сильно упрощает повторение настройки на другой машине.
* Для командной работы добавьте этот README в репозиторий и при необходимости
  дополняйте разделами:

    * «Как запускать готовый бинарник на Raspberry Pi»;
    * «Настройка отладки через GDB»;
    * «Примеры кросс-сборки конкретных целей CMake».
