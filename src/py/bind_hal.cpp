...
void bind_hal(py::module_& module) {
	py::class_<RadioLibHal, PyHal, std::shared_ptr<RadioLibHal>>(module, "RadioLibHal")
		...
		throw std::runtime_error("attachInterrupt is only meaningful in concrete HAL implementations"); }, py::arg("interruptNum"), py::arg("callback"), py::arg("mode"), R"(Для Python-наследников реализуйте attachInterrupt в своём классе.)")
